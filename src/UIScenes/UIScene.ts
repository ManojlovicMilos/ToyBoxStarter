...
class UIScene extends TBX.Scene2D {
    ...
    protected CreateBackground(Name: string): void {
        let Back:TBX.Tile = TBX.SceneObjectUtil.CreateTile(Name, ["/Textures/Backgrounds/"+Name+".png"], new TBX.Vertex(960, 540, 0), new TBX.Vertex(1920, 1080, 1)); 
        this.Attach(Back);
    }
    ...
    protected CreateLabel(Text: string, Order: number): TBX.UI.Label {
        ...
        Label.Position = new TBX.Vertex(0, -200 + 60 * Order, 0.2);
    ...
    protected CreateSlider(Text: string, Value: number, Order: number): Slider {
        let NewSlider: Slider = new Slider(Text, Value);
        NewSlider.Name = Text;
